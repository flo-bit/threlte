...
import { AdditiveBlending, CircleGeometry, Color, DoubleSide, HalfFloatType, InstancedMesh, LinearFilter, LinearMipMapLinearFilter, MeshBasicMaterial, NearestFilter, Object3D, RGBAFormat, ShaderMaterial, UnsignedIntType, Vector2, Vector3, Vector4, WebGLRenderTarget } from 'three'}
...
class LensFlarePass extends Pass {
  constructor(scene, camera, flares, { coverageScale = 1 } = {}) {
    ...
    this.scene = scene
    this.camera = camera
    this.flareRenderTarget = new WebGLRenderTarget(this.width / 2, this.height / 2, {
      minFilter: LinearFilter,
      magFilter: LinearFilter,
      format: RGBAFormat,
      type: HalfFloatType
    })
    this.coverageTarget = new WebGLRenderTarget(32, 32, {
      type: HalfFloatType,
    ...
    this.transparencyRenderTargetDWTrue.depthTexture = new DepthTexture(
      this.width,
      this.height,
      UnsignedIntType
    )
    ...
    this.transparencyRenderTargetDWFalse.depthTexture = new DepthTexture(
      this.width,
      this.height,
      UnsignedIntType
    )
    ...
    this.outputQuad = new FullScreenQuad(
      new ShaderMaterial({
        uniforms: {
          ...
          texelSize: { value: new Vector4(0, 0, 0, 0) }
        },
        vertexShader: `
							varying vec2 vUv;
							void main() {
...
vec2 p3 = (vec2(iuv.x + h1x, iuv.y + h1y) - 0.5) * texelSize.xy;
    ...
    const flareTransparentBlockers = new InstancedMesh(
      new CircleGeometry(1.25 * coverageScale, 32),
...
c.r+=f1+f2+f4+f5+f6; c.g+=f1+f22+f42+f52+f62; c.b+=f1+f23+f43+f53+f63;
  ...
  renderTransparency(renderer) {
    ...
    for (let i = 0; i < this.flares.length; i++) {
      ...
      dummy.lookAt(this.camera.getWorldPosition(new Vector3()))
    ...
    for (let i = 0; i < flaresToRender.length; i += 4) {
      ...
      if (batch.length < 4) {
        ...
        for (let j = batch.length; j < 4; j++) {
          this.coverageQuad.material.uniforms.lensPosition.value[j] = new Vector3(0, 0, 0)
        }
      }
      ...
      if (batch.length < 4) {
        ...
        for (let j = batch.length; j < 4; j++) {
          ...
          this.flareQuad.material.uniforms.lensPosition.value[j] = new Vector3(0, 0, 0)
        }
      }
      ...
      this.flareQuad.material.uniforms.cameraPos.value.copy(
        this.camera.getWorldPosition(new Vector3())
      )
